...
RSpec.describe do
  ...
  it 'passes with the day 3 sample data slope 5' do
    input = File.readlines('sample.txt')
    expect(Part2.run_slope(input, 1, double_move: true)[:count]).to eq(2)
    expect(Part2.run_slope(input, 1, double_move: true)[:values]).to eq([1, 0, 1, 0, 0])
  end

  it 'passes with the day 3 sample data' do
    input = File.readlines('sample.txt')
    slopes = [{ move: 1 }, { move: 3 }, { move: 5 }, { move: 7 }, { move: 1, double_move: true }]
